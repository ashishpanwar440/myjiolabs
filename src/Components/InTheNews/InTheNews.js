...
export default class InTheNews extends React.Component {
    constructor (props) {
        ...
        this.navToNews = this.navToNews.bind (this)   
        this.currNewsIndex = 0     
    ...
    animate () {
        let newNewsItem = document.getElementById ('news-0')
        let newDot = document.getElementById ('newsDot-0')

        newNewsItem.classList.remove ('hideNewsItemToLeft', 'hideNewsItemToRight')
        newNewsItem.classList.add ('showNewsItem')
        newDot.classList.add ('selectedNewsDot')

        this.clearNewsIntervalID = setInterval (() => {
            this.navToNews ((this.currNewsIndex + 1) % this.news.length)
        }, 4000)
    }

    navToNews (newIndex) {
        let oldIndex = this.currNewsIndex
        if (oldIndex !== newIndex) {
            
            let newNewsItem = document.getElementById ('news-' + newIndex)
            let oldNewsItem = document.getElementById ('news-' + oldIndex)
            let newDot = document.getElementById ('newsDot-' + newIndex)
            let oldDot = document.getElementById ('newsDot-' + oldIndex)
            
            newNewsItem.classList.remove ('hideNewsItemToLeft', 'hideNewsItemToRight')
            oldNewsItem.classList.remove ('showNewsItemFromLeft', 'showNewsItemFromRight')
            
            oldDot.classList.remove ('selectedNewsDot')
            newDot.classList.add ('selectedNewsDot')
            
            if (oldIndex < newIndex) {
                newNewsItem.classList.add ('showNewsItemFromRight')
                oldNewsItem.classList.add ('hideNewsItemToLeft')
            } else {
                newNewsItem.classList.add ('showNewsItemFromLeft')
                oldNewsItem.classList.add ('hideNewsItemToRight')
            }

            this.currNewsIndex = newIndex
        }
    }
                    ...
                    <div id = 'legacyCenter'>
                        ...
                        {this.legacies.map ((legacy, index) => <div 
                            ...
                            <FontAwesomeIcon 
                                ...
                                className = 'closeCent' 
                                onClick = {() => {
                                    let center = document.querySelector ('#legCent-' + index)
                                    center.classList.remove ('showCenter')
                                }}    
                            />

                            <div className = 'legCentHead'>
                                {legacy.head}
                            </div>
                            <div className = 'legCentImgCont'>
                                <img
                                    src = {legacy.imgSrc}
                                    alt = 'leg-img'
                                    className = 'legCentImg'
                                />
                            </div>
                            <div className = 'legCentText'>
                                 {legacy.text}
                            </div>
                            <div className = 'legCentLinkCont'>
                                <a 
                                    className = 'legCentLink'
                                    href = {legacy.link}
                                    target = '_blank'
                                >
                                    Visit
                ...
                <div id = 'newsCont'>
                    ...
                    <div id = 'newsGrid'>
                        {this.news.map ((news, index) => <div
                            className = 'newsItem hideNewsItemToRight'
                            index = {index}
                            key = {index}
                            id = {'news-' + index}
                        >
                    ...
                    <div id = 'newsDots'>
                        {this.news.map((news, index) => <div
                            key = {index}
                            className = 'newsDotCont'
                        >
                            <div 
                                className = 'newsDot'
                                id = {'newsDot-' + index}
                                onClick = {() => {
                                    this.navToNews (index)
                                }}
                            ></div>
                        </div>)}
                    </div>
    ...
    componentWillUnmount () {
        clearInterval (this.clearNewsIntervalID)
    }
}